...
lookup_device_settings (ClutterInputDevice *device)
{
  ...
  if (type == CLUTTER_TOUCHSCREEN_DEVICE)
    ...
    {
      group = "tablets";
      schema = "io.github.scarecrow_de.desktop.peripherals.tablet";
    }
  else
    return NULL;
  ...
  path = g_strdup_printf ("/io/github/scarecrow_de/desktop/peripherals/%s/%s:%s/",
                      ...
                      ClutterInputDevice     *device)
{
  ...
  if (serial == 0 || serial == 1)
    ...
    {
      path = g_strdup_printf ("/io/github/scarecrow_de/desktop/peripherals/stylus/%" G_GINT64_MODIFIER "x/",
                              serial);
    }

  tool_settings =
    g_settings_new_with_path ("io.github.scarecrow_de.desktop.peripherals.tablet.stylus",
                              path);
                            ...
                            gint                mode)
{
  ...
  g_string_append_printf (path, "/io/github/scarecrow_de/desktop/peripherals/tablets/%s:%s/%s%c",
...
meta_input_settings_init (MetaInputSettings *settings)
{
  ...
  priv->mouse_settings = g_settings_new ("io.github.scarecrow_de.desktop.peripherals.mouse");
  g_signal_connect (priv->mouse_settings, "changed",
                    G_CALLBACK (meta_input_settings_changed_cb), settings);

  priv->touchpad_settings = g_settings_new ("io.github.scarecrow_de.desktop.peripherals.touchpad");
  g_signal_connect (priv->touchpad_settings, "changed",
                    G_CALLBACK (meta_input_settings_changed_cb), settings);

  priv->trackball_settings = g_settings_new ("io.github.scarecrow_de.desktop.peripherals.trackball");
  g_signal_connect (priv->trackball_settings, "changed",
                    G_CALLBACK (meta_input_settings_changed_cb), settings);

  priv->keyboard_settings = g_settings_new ("io.github.scarecrow_de.desktop.peripherals.keyboard");
